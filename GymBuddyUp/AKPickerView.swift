...
public class AKPickerView: UIView, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, AKCollectionViewLayoutDelegate {
    ...
    public lazy var highlightedFont = UIFont.boldSystemFontOfSize(24)
    
    /// Readwrite. A color of the text on NOT selected cells.
    @IBInspectable public lazy var textColor: UIColor = UIColor.flatGrayColor()
    
    /// Readwrite. A color of the text on selected cells.
    @IBInspectable public lazy var highlightedTextColor: UIColor = UIColor.flatBlackColorDark()
    ...
    public func reloadData() {
        ...
        if self.dataSource != nil && self.dataSource!.numberOfItemsInPickerView(self) > 0 {
            self.selectItem(dataSource!.numberOfItemsInPickerView(self)/2, animated: true, notifySelection: true)
    ...
    /* jump to item by Yi Huang*/
    public func jumpToItem(item: Int, animated: Bool = false) {
        self.jumpToItem(item, animated: animated, notifySelection: true)
    }
    
    private func jumpToItem(item: Int, animated: Bool, notifySelection: Bool) {
